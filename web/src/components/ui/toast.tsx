...
const toastVariants = cva(
	'group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-2xl p-3 pr-8 transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--kb-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--kb-toast-swipe-move-x)] data-[swipe=move]:transition-none data-opened:animate-in data-closed:animate-out data-[swipe=end]:animate-out data-closed:fade-out-80 data-closed:slide-out-to-right-full data-opened:slide-in-from-top-full sm:data-opened:slide-in-from-bottom-full',
	{
		variants: {
			variant: {
...
const ToastClose = <T extends ValidComponent = 'button'>(
	...
	return (
		<ToastPrimitive.CloseButton
			class={cn(
				'absolute right-2 top-2 rounded-2xl p-1 text-foreground/50 opacity-0 transition-opacity focus:opacity-100 focus:outline-hidden focus:ring-2 group-hover:opacity-100 group-[.destructive]:text-destructive-foreground group-[.error]:text-error-foreground group-[.success]:text-success-foreground group-[.warning]:text-warning-foreground',
				local.class,
			)}
			{...others}
		>
